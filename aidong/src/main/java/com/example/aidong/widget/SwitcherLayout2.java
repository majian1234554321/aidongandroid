...
public class SwitcherLayout2 {
    ...
    private final String[] mSupportedViewGroups = new String[]{"linearlayout", "relativelayout",
            "framelayout", "scrollview", "recyclerview", "viewgroup","ptrframelayout","swiperefreshlayout","CustomRefreshLayout","customrefreshlayoutptr"};

    public SwitcherLayout2(Context context, View targetView) {
        ...
        String viewType = viewClass.getName().substring(viewClass.getName().
                lastIndexOf('.') + 1).toLowerCase(Locale.getDefault());
        String superViewType = superViewClass.getName().substring(superViewClass.getName()
                .lastIndexOf('.') + 1).toLowerCase(Locale.getDefault());

        if (Arrays.asList(mSupportedAbsListViews).contains(viewType) ||
                Arrays.asList(mSupportedAbsListViews).contains(superViewType)) {
            initializeAbsListView();
        } else if (Arrays.asList(mSupportedViewGroups).contains(viewType) ||
                Arrays.asList(mSupportedViewGroups).contains(superViewType)) {
            initializeViewCroup();
        } else {
            throw new IllegalArgumentException("TargetView type [" + superViewType + "] " +
                    "is not supported !");
        }

    }

    public SwitcherLayout2(Context context, int viewID) {
        this.mContext = context;
        this.mInflater = ((Activity) mContext).getLayoutInflater();
